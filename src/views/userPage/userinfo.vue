      ...
      <Tabs class="tab-box">
        ...
        <TabPane label="订单中心" icon="md-cart">
          <Row style="padding:5px 0px;">
            <Col span="11">
            <el-date-picker v-model="selectDate" size="small" type="datetimerange" :picker-options="pickerOptions" range-separator="至" start-placeholder="开始日期" format="yyyy/MM/dd HH:mm:ss" end-placeholder="结束日期" align="right">
            </el-date-picker>
            </Col>
            <Col span="13">
          ...
          <!-- 表格数据 -->
          <el-table :data="myorders" class="myorders-table" style="width: 100%" border show-summary :default-sort="{prop: 'date', order: 'descending'}">
            <el-table-column type="index" label="序号" align="center" width="60">
            </el-table-column>
            <el-table-column label="订单号" align="center" width="280">
              <template slot-scope="scope">
                <span style="color:#E9232C;">{{scope.row._id}}</span>
              </template>
            </el-table-column>
            <el-table-column label="购买类型" align="center" width="200">
              <template slot-scope="scope">
                <span style="color:#2d8cf0;">{{scope.row.courseType}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="totalMoney" label="总金额" sortable align="center" width="130">
            </el-table-column>
            <el-table-column label="下单时间" sortable align="center" width="260">
              <template slot-scope="scope">
                <Icon type="md-time" />
      ...
      <Collapse simple id="mobile-box" accordion>
        ...
        <Panel name="3" :hide-arrow="true">
          ...
          <p slot="content" style="padding-top:10px;">
            <Row style="padding:5px 0px;">
              <Col span="11">
              <el-date-picker v-model="selectDate2" style="width:320px;overflow:scroll;" size="mini" type="daterange" range-separator="至" start-placeholder="开始日期" format="yyyy/MM/dd HH:mm:ss" end-placeholder="结束日期" align="right">
              </el-date-picker>
              </Col>
            </Row>
            <el-table :data="myorders" class="myorders-table" style="width: 100%;" border :default-sort="{prop: 'date', order: 'descending'}">
              <el-table-column label="创建时间" sortable align="center" width="220">
                <template slot-scope="scope">
                  <Icon type="md-time" />
                  <span style="margin-left:10px;">{{scope.row.buyTime}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="operation" align='center' label="查看" width="80">
                <template slot-scope='scope'>
            ...
            <Row>
              ...
              <el-pagination background v-if='paginations.total > 0' :page-sizes="paginations.page_sizes" :page-size="paginations.page_size" :total="paginations.total" layout="prev, pager, next, total" @current-change='handleCurrentChange' @size-change='handleSizeChange'>
              </el-pagination>
              </Col>
            </Row>
            <Row>
      ...
      <Modal v-model="editmodal" width="400">
        ...
        <el-form :model="newUserInfo" :rules="editRule" ref="editForm" label-width="100px" class="editForm">
          <el-form-item label="换头像:" prop="avatar">
            <Upload :show-upload-list="false" :format="['jpg','jpeg','png']" :max-size="10000" multiple type="drag" :on-success="handleSuccess" :action="'http://localhost:3001/api/user/upload?email='+getUserInfo.email" style="display:block;width:44px;">
...
export default {
  ...
  methods: {
    ...
    getMyOrders() {
      ...
      this.$axios
        .post("http://localhost:3001/api/admin/logs/user/email", {
          email: email
        })
        .then(res => {
    ...
    submitnewpwdForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$axios
            .post("http://localhost:3001/api/user/modpwd", params)
            .then(res => {
              if (res) {
    ...
    submitbuy() {
      var pay = 0;
      this.buycourseInfo.javascript.hasbuy
        ? (pay += this.buycourseInfo.javascript.price)
        : (pay += 0);
      this.buycourseInfo.html5.hasbuy
        ? (pay += this.buycourseInfo.html5.price)
        : (pay += 0);
      this.buycourseInfo.css.hasbuy
        ? (pay += this.buycourseInfo.css.price)
        : (pay += 0);
      if (pay > 0) {
        if (pay > this.getUserInfo.money) {
          ...
          this.$Modal.confirm({
            ...
            onOk: () => {
              ...
              buy.totalMoney = pay;
              this.$axios
                .post("http://localhost:3001/api/user/updata/token", buy)
                .then(res => {
    ...
    submitEdit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.isUpload == true
            ? ""
            : (this.newUserInfo.avatar = this.$store.getters.avatar);
          console.log(this.newUserInfo);
          this.$axios
            .post(
              "http://localhost:3001/api/user/updata/token",
              this.newUserInfo
            )
            .then(res => {
              localStorage.setItem("jwtToken", res.data.token);
              ...
              this.userAvatar =
                "http://localhost:3001/api/user/avatar?email=" + decoded.email;
    ...
    calcelEdit() {
      this.isUpload = false;
      this.newUserInfo.avatar =
        "http://localhost:3001/api/user/avatar?email=" +
    ...
    handleSuccess(res, file) {
      ...
      this.editAvatar =
        "http://localhost:3001/api/user/avatar/back?email=" +
        res.result.email +
        "&&avatar=" +
        res.result.avatar;
    },
  ...
  created() {
    ...
    const start = parseInt(
      this.$store.getters.user.startdate.split(" ")[0].split("-")[2]
    );
    ...
    if (this.$store.getters.user.enddate == 0) {
      ...
      if (start > curdate) {
        ...
        if (this.timeout == 0) {
          ...
          this.timeout = "已过期";
          this.$axios
            .post("http://localhost:3001/api/user/updata/token", unlimit)
            .then(res => {
              localStorage.setItem("jwtToken", res.data.token);
              const decode = jwt_decode(res.data.token);
              this.$store.commit("user", decode);
